...
const token = (data) => {
    let encodedToken = jwt.sign(data, 'ini rahasia',{ expiresIn: '1h' })
    return encodedToken
};
async function decoded (token) {
    try {
        let decodedToken = jwt.verify(token, 'ini rahasia');
        return decodedToken
    } catch (error) {
        if(error.name==='JsonWebTokenError'){
            throw new ApolloError('Please Relogin', 'UN_AUTHENTICATED'); 
        }
...
function generateRandomPlaylist(playLists,limitTime){
    let totalDuration = 0
    let groupedMusics=[]
    for (let i = playLists.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        let shuffledPlaylistTemp = playLists[i]
        playLists[i] = playLists[j]
        playLists[j] = shuffledPlaylistTemp
    }
    for (let song of playLists) {
        let [minutes, seconds] = song.duration.split(':').map(Number); // Split and convert to numbers
        let durationInSeconds = minutes * 60 + seconds; // Calculate total duration in seconds
        totalDuration += durationInSeconds
        if (totalDuration < limitTime) {
...
module.exports = { bookPurchasing, validateInput, generateCreditDates, generatePrices, loopingText,token,decoded,generateRandomPlaylist }