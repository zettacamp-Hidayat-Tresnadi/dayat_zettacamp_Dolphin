...
async function decodedWebhook (token) {
    try {
        let decodedToken = jwt.verify(token, 'ini rahasia');
        return decodedToken
    } catch (error) {
...
module.exports = { bookPurchasing, validateInput, generateCreditDates, generatePrices, loopingText,token,decoded,decodedWebhook }