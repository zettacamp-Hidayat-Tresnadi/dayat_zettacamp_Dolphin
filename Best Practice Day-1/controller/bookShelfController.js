...
class BookshelfController {
    static async createBookShelf(bookshelfForm) {
        try {
            const { name, books } = bookshelfForm
    ...
    static async getOneBookShelf(bookshelfId) {
        const bookshelf = await BookShelf.findOne({ _id: bookshelfId })
        if (!bookshelf) {
            throw { message: 'Bookshelf is not found' }
        }
        return bookshelf
    ...
    static async getAllBookshelves(bookIds) {
        let filter = {}
        if (bookIds.length > 0) {
            filter.books = {
                $in: bookIds,
        ...
        let matchedBooks = await BookShelf.find(filter)
        // .
    ...
    static async unwindFunction(_, res, next) {
        try {
    ...
    static async lookUp(_, res, next) {
        try {
            let booksofEachBookShelf = await BookShelf.aggregate([
                {
                    $lookup: {
