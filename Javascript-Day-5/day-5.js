...
function generateCreditDates(creditTerm) {
    // Credit Term
    let today = new Date()
    // today.setDate(31)
    let messages = []
    ...
    messages = dueDates.map((date, index) => {
        if (index === 0) {
            message = `The due date for next month is :${date}`
        }
        else {
            message = `The due date for next month from ${newDate} is at this day : ${date}`
        }
        newDate=date
        return message
    })
...
function bookPurchasing(book, discountPercentage, taxPercentage, amountOfStock, amountOfPurchasedBook, creditTerm) {
    ...
    let creditDates = generateCreditDates(creditTerm)
    ...
    response += ` If paid in installments, the price is ${(totalPrice/creditTerm).toFixed(2)} this is a list of dates : `
    // creditDates.unshift(response)
    console.log(response)
    return creditDates
}
console.log(bookPurchasing(book, 25, 10, 4, 6,8))




// console.log(generateCreditDates(4))
